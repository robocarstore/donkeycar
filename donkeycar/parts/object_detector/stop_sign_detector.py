...
class StopSignDetector(object):
    ...
    def __init__(self, min_score, show_bounding_box, max_reverse_count, debug=False):  
        ...
        self.maxReverseCount = max_reverse_count
        self.reverseCount = max_reverse_count
    ...
    def run(self, img_arr, throttle, debug=False):
        ...
        if traffic_light_obj or self.isReversing:
            ...
            if self.reverseCount < self.maxReverseCount:
                self.isReversing = True
                self.reverseCount += 1
                print(f"reversing, reverseCount = {self.reverseCount}/{self.maxReverseCount}")
                return -1, img_arr
            else:
                self.isReversing = False
                return 0, img_arr                   
